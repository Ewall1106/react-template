export const components = [
    ...
    response: (config) => {
      return {
        ...
        data: [
          {
            componentId: "1",
            componentName: "@basic-assets/basic-text",
            componentCdn:
              "https://cdn.jsdelivr.net/npm/@bamboo-assets/basic-text/lib/index.js",
            componentType: "basic",
            componentChild: [],
            props: {
              text: "按钮组件",
  ...
  {
    url: "/prod-api/search/result",
    type: "post",
    response: (config) => {
      const { pageSize, pageNo } = JSON.parse(config.body);
